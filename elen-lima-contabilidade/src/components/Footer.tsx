import { SITE_NAME, INSTAGRAM_URL } from '@/config/constants'

export default function Footer() {
  const year = new Date().getFullYear()

  return (
    <footer className="bg-accent px-4 py-10 sm:px-6 lg:px-8" role="contentinfo">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col items-center justify-between gap-6 border-b border-white/10 pb-8 sm:flex-row">
          {/* Logo / Nome + Redes Sociais */}
          <div>
            <p className="text-lg font-bold text-white">
              {SITE_NAME.split(' ')[0]} {SITE_NAME.split(' ')[1]}{' '}
              <span className="text-primary-light">{SITE_NAME.split(' ')[2]}</span>
            </p>
            <p className="mt-1 text-sm text-white/60">
              Especialista em Arquitetos e Engenheiros — Contagem/MG
            </p>
            <div className="mt-3 flex gap-3">
              <a
                ...
                aria-label={`Siga ${SITE_NAME} no Instagram`}
        ...
        <p className="text-center text-xs text-white/40">
          &copy; {year} Elen Lima Contabilidade. Todos os direitos reservados.
